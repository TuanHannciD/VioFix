...
namespace AppView.Controllers
{
    public class ThongKeController : Controller
    {
        ...
        public async Task<IActionResult> ThongKeSP()
        {
            ...
            var PhongCachs = JsonConvert.DeserializeObject<List<ThongKeMSSanPhamTheoSoLuong>>(apiData);

            return View(PhongCachs);
        }
        [HttpGet]
        public async Task<IActionResult> ThongKeSPTheoThang(DateTime? ngay, DateTime? thang, DateTime? nam)
        {
            ...
            string apiData = await response.Content.ReadAsStringAsync();
            var PhongCachs = JsonConvert.DeserializeObject<List<ThongKeMSSanPhamTheoSoLuong>>(apiData);
            var timkiem = PhongCachs.ToList();

            if (ngay.HasValue)
            {
                timkiem = PhongCachs.Where(x => x.Ngay.Date == ngay.Value.Date && x.Ngay.Month == ngay.Value.Month && x.Ngay.Year == ngay.Value.Year).ToList();
            }
            if (thang.HasValue)
            {
                timkiem = PhongCachs.Where(x => x.Ngay.Month == thang.Value.Month && x.Ngay.Year == thang.Value.Year).ToList();
            }
            if (nam.HasValue)
            {
                timkiem = PhongCachs.Where(x => x.Ngay.Year == nam.Value.Year).ToList();
            }
        ...
        public async Task<IActionResult> ThongKeKH(int ProductPage = 1)
        {
            ...
            var response = await _httpClient.GetAsync(apiUrl);
            string apiData = await response.Content.ReadAsStringAsync();
            var PhongCachs = JsonConvert.DeserializeObject<List<ThongKeKHMuaNhieu>>(apiData);
        ...
        public async Task<IActionResult> ThongKeDoanhThuTheoNgay()
        {
            ...
            var response = await _httpClient.GetAsync(apiUrl);
            string apiData = await response.Content.ReadAsStringAsync();
            var PhongCachs = JsonConvert.DeserializeObject<List<ThongKeDoanhThu>>(apiData);
        ...
        public async Task<IActionResult> ThongKe()
        {
            ...
            var PhongCachs = JsonConvert.DeserializeObject<List<ThongKeKHMuaNhieu>>(apiData4);
            var timkiem = PhongCachs.ToList();
