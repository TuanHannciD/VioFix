    ...
    public class KhuyenMaiController : ControllerBase
    {
        ...
        public async Task<List<AllViewCTSP>> GetAllCTSPBySp(Guid idSanPham)
        {
            ...
            var AllCTSP = await (from CTSP in _dbcontext.ChiTietSanPhams.AsNoTracking()
                                 join dungtich in _dbcontext.DungTichs.AsNoTracking() on CTSP.IDDungTich equals dungtich.ID
                                 join size in _dbcontext.NhomHuongs.AsNoTracking() on CTSP.IDNhomHuong equals size.ID
                                 ...
                                 select new AllViewCTSP()
                                 {
                                     ...
                                     MaCTSP=CTSP.Ma,
                                     TenSanPham = sp.Ten,
                                     TenAnh = (from anh in _dbcontext.Anhs where sp.ID == anh.IDSanPham && dungtich.ID == anh.IDDungTich select anh.DuongDan).FirstOrDefault(),
                                     IdKhuyenMai = (from km in _dbcontext.KhuyenMais where CTSP.IDKhuyenMai == km.ID select CTSP.IDKhuyenMai).FirstOrDefault(),
                                     TenDungTich = dungtich.Ten,
                                     MaDungTich=dungtich.Ma,
                                     TenNhomHuong = size.Ten,
                                     GiaGoc = CTSP.GiaBan,
        ...
        public List<AllViewSp> GetAllSP()
        {
            ...
            var result = _dbcontext.SanPhams
                            ...
                            .Join(_dbcontext.PhongCachs, sp => sp.IDPhongCach, cl => cl.ID, (sp, cl) => new { sp_cl = sp, phongcachs = cl })
                            ...
                            .Select(group => new AllViewSp {
                                ...
                                IDPhongCach = group.FirstOrDefault().sp_cl_lsp_ctsp.sp_cl_lsp.phongcachs.ID,
        ...
        [Route("GetAllSPByKmLoaiSPPhongCach")]
        [HttpGet]

        public async Task<List<AllViewSp>> GetAllSPByKm(Guid? idkm, Guid? idLoaiSP, Guid? idPhongCach)
        {
            if (!_dbcontext.KhuyenMais.Any(c => c.ID == idkm) && !_dbcontext.LoaiSPs.Any(c => c.ID == idLoaiSP) && !_dbcontext.PhongCachs.Any(y => y.ID == idPhongCach)) throw new Exception($" khong tim thay san pham co id:{idkm},{idLoaiSP},{idPhongCach}");
            var AllCTSP = (from SP in _dbcontext.SanPhams.AsNoTracking()
                           ...
                           join phongcach in _dbcontext.PhongCachs.AsNoTracking() on SP.IDPhongCach equals phongcach.ID
                           ...
                           select new { SP, anh, loaisp, phongcach, CTSP, km });
            ...
            if (!string.IsNullOrEmpty(idPhongCach.ToString()))
            {
                AllCTSP = AllCTSP.AsNoTracking().Where(x => x.phongcach.ID == idPhongCach);
            }
        ...
        [Route("GetAllSPNoKMByLoaiSPPhongCach")]
        [HttpGet]

        public List<AllViewSp> TKGetAllSPNoKmByLoaiSPPhongCach(Guid id, Guid? idLoaiSP, Guid? idPhongCach)
        {
            if (!_dbcontext.LoaiSPs.Any(c => c.ID == idLoaiSP) && !_dbcontext.PhongCachs.Any(y => y.ID == idPhongCach)) throw new Exception($" khong tim thay san pham co id:{idLoaiSP},{idPhongCach}");
            //var AllCTSP = (from SP in _dbcontext.SanPhams.AsNoTracking()
            //               join anh in _dbcontext.Anhs.AsNoTracking() on SP.ID equals anh.IDSanPham
            //               join loaisp in _dbcontext.LoaiSPs.AsNoTracking() on SP.IDLoaiSP equals loaisp.ID
            //               join phongcach in _dbcontext.PhongCachs.AsNoTracking() on SP.IDPhongCach equals phongcach.ID
            ...
            //               select new { SP, anh, loaisp, phongcach, CTSP });
            ...
            if (!string.IsNullOrEmpty(idPhongCach.ToString()))
            {
                result = result.Where(x => x.IDPhongCach == idPhongCach).ToList();
            }
            return result;
        }
