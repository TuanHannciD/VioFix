    ...
    public class SanPhamService : ISanPhamService
    {
        ...
        public async Task<bool> UpdateSanPham(SanPhamUpdateRequest request)
        {
            try
            {
                ...
                NhomHuong? chatLieu = _context.NhomHuongs.FirstOrDefault(x => x.Ten == request.TenChatLieu);
                if (loaiSPCon == null)
                {
                ...
                if (chatLieu == null)
                {
                    chatLieu = new NhomHuong() { ID = Guid.NewGuid(), Ten = request.TenChatLieu, TrangThai = 1 };
                    await _context.AddAsync(chatLieu);
                }
                sanpham.Ten = request.Ten;
                sanpham.MoTa = request.MoTa;
                sanpham.IDNhomHuong = chatLieu.ID;
        ...
        public async Task<SanPhamUpdateRequest> GetSanPhamById(Guid id)
        {
            try
            {
                ...
                var response = new SanPhamUpdateRequest()
                {
                    ...
                    Ten = sanPham.Ten,
                    MoTa = sanPham.MoTa,
                    TenChatLieu = _context.NhomHuongs.First(x => x.ID == sanPham.IDNhomHuong).Ten,
            ...
            try
            {
                ...
                var lstSanPham = (from a in _context.SanPhams
                                  ...
                                  select new SanPhamViewModelAdmin()
                                  {
                                      ...
                                      LoaiSPCon = e.Ten,
                                      Anh = sp == null ? "" : _context.Anhs.First(x => x.IDPhongCach == sp.IDPhongCach && x.IDSanPham == a.ID).DuongDan,
                                      ChatLieu = _context.NhanViens.First(x => x.ID == a.IDNhomHuong).Ten,
            ...
            try
            {
                ...
                var lstSanPham = await (from a in _context.SanPhams.Where(x => x.TrangThai == 1)
                                        ...
                                        select new SanPhamViewModel()
                                        {
                                            ...
                                            Image = _context.Anhs.First(x => x.IDPhongCach == b.IDPhongCach && x.IDSanPham == a.ID).DuongDan,
                                            IDMauSac = b.IDPhongCach,
                                            IDKichCo = b.IDDungTich,
                                            IDChatLieu = a.IDNhomHuong,
                                            GiaGoc = b.GiaBan,
                                            SoLuong = b.SoLuong,
        ...
        public async Task<ChiTietSanPhamUpdateRequest> AddSanPham(SanPhamRequest request)
        {
            try
            {
                ...
                NhomHuong? chatLieu = _context.NhomHuongs.FirstOrDefault(x => x.Ten.ToUpper() == request.TenChatLieu.Trim().ToUpper());
                if (loaiSPCon == null)
                {
                ...
                if (chatLieu == null)
                {
                    chatLieu = new NhomHuong() { ID = Guid.NewGuid(), Ten = request.TenChatLieu.Trim(), TrangThai = 1 };
                    await _context.AddAsync(chatLieu);
                }
                var max = 0;
                ...
                SanPham sanPham = new SanPham() { ID = Guid.NewGuid(), Ten = request.Ten, Ma = "SP" + (max + 1), MoTa = request.MoTa, TrangThai = 1, IDLoaiSP = loaiSPCon.ID, IDNhomHuong = chatLieu.ID };
                await _context.SanPhams.AddAsync(sanPham);
                await _context.SaveChangesAsync();
                foreach (var x in request.PhongCachs)
                {
        ...
        public async Task<bool> AddAnhToSanPham(List<AnhRequest> request)
        {
            try
            {
                ...
                {
                    Anh anh = new Anh() { ID = Guid.NewGuid(), DuongDan = x.DuongDan, IDSanPham = x.IDSanPham, IDPhongCach = _context.PhongCachs.First(y => y.Ma == x.MaMau).ID, TrangThai = 1 };
        ...
        public List<AnhViewModel> GetAllAnhSanPham(Guid idSanPham)
        {
            try
            {
                var lst = (from a in _context.Anhs.Where(x => x.IDSanPham == idSanPham)
                           join b in _context.PhongCachs on a.IDPhongCach equals b.ID
        ...
        public async Task<ChiTietSanPhamUpdateRequest> AddChiTietSanPham(ChiTietSanPhamAddRequest request)
        {
            try
            {
                ...
                List<ChiTietSanPhamRequest> lst = new List<ChiTietSanPhamRequest>();
                List<PhongCach> mauSac = new List<PhongCach>();
                ...
                foreach (var x in request.PhongCachs)
                {
                    ...
                    if (_context.Anhs.FirstOrDefault(item => item.IDSanPham == request.IDSanPham && item.IDPhongCach == _context.PhongCachs.First(z => z.Ma == x.Ma).ID) == null)
                    {
                        mauSac.Add(x);
                    }
                }
                return new ChiTietSanPhamUpdateRequest() { IDSanPham = request.IDSanPham, ChiTietSanPhams = lst.GroupBy(x => new { MauSac = x.IDMauSac, KichCo = x.IDKichCo }).Select(y => y.First()).ToList(), Location = 1, PhongCachs = mauSac, Ma = _context.SanPhams.First(x => x.ID == request.IDSanPham).Ma };
            }
            catch { return new ChiTietSanPhamUpdateRequest(); }
        }
        public ChiTietSanPhamViewModel? GetChiTietSanPhamByID(Guid id)
        {
            try
            {
                ...
                if (temp != null)
                {
                    var anh = _context.Anhs.FirstOrDefault(x => x.IDPhongCach == temp.IDPhongCach && x.IDSanPham == temp.IDSanPham);
                    ChiTietSanPhamViewModel chiTietSanPham = new ChiTietSanPhamViewModel()
                    {
                        ...
                        MauSac = _context.PhongCachs.First(x => x.ID == temp.IDPhongCach).Ten,
                        KichCo = _context.DungTichs.First(x => x.ID == temp.IDDungTich).Ten
                    };
        ...
        public async Task<ChiTietSanPhamViewModelHome> GetAllChiTietSanPhamHome(Guid idSanPham)
        {
            try
            {
                ...
                List<PhongCach> mauSacs = new List<PhongCach>();
                List<DungTich> kichCos = new List<DungTich>();
                foreach (var x in lstChiTietSanPham)
                {
                    mauSacs.Add(_context.PhongCachs.FindAsync(x.IDPhongCach).Result);
                    kichCos.Add(_context.DungTichs.FindAsync(x.IDDungTich).Result);
                }
                ...
                {
                    chiTietSanPham.Anhs.Add(new AnhRequest() { DuongDan = item.DuongDan, MaMau = item.IDPhongCach.ToString() });
                }
                ...
                var sptt = await (from sp in _context.SanPhams.Where(p => p.IDLoaiSP == sanPham.IDLoaiSP && p.ID != idSanPham)
                                  ...
                                  select new SanPhamTuongTuViewModel()
                                  {
                                      ...
                                      TenSP = sp.Ten,
                                      GiaSPTT = ctsp.GiaBan,
                                      DuongDanSPTT = _context.Anhs.FirstOrDefault(x => x.IDPhongCach == ms.ID && x.IDSanPham == sp.ID).DuongDan,
                                  }).Take(5).ToListAsync();
        ...
        public async Task<ChiTietSanPhamRequest?> CreateChiTietSanPhamFromSanPham(PhongCach mauSacRequest, string tenKichCo, List<ChiTietSanPham>? chiTietSanPhams)
        {
            try
            {
                var mauSac = _context.PhongCachs.FirstOrDefault(x => x.Ma == mauSacRequest.Ma);
                if (mauSac == null)
                {
                    mauSac = new PhongCach() { ID = Guid.NewGuid(), Ten = mauSacRequest.Ten, Ma = mauSacRequest.Ma.ToLower(), TrangThai = 1 };
                    _context.Add(mauSac);
                }
                var kichCo = _context.DungTichs.FirstOrDefault(x => x.Ten.ToUpper() == tenKichCo.Trim().ToUpper());
                if (kichCo == null)
                {
                    kichCo = new DungTich() { ID = Guid.NewGuid(), Ten = tenKichCo.Trim(), TrangThai = 1 };
                    _context.Add(kichCo);
                ...
                {
                    var item = chiTietSanPhams.FirstOrDefault(x => x.IDPhongCach == mauSac.ID && x.IDDungTich == kichCo.ID);
                    if (item == null)
                    {
                        var chiTietSanPham = new ChiTietSanPhamRequest() { IDChiTietSanPham = Guid.NewGuid(), SoLuong = 0, GiaBan = 0, IDMauSac = mauSac.ID.Value, IDKichCo = kichCo.ID, MaMau = mauSac.Ma, TenMauSac = mauSac.Ten, TenKichCo = kichCo.Ten };
                        return chiTietSanPham;
        ...
        public async Task<bool> AddChiTietSanPhamFromSanPham(ChiTietSanPhamUpdateRequest request)
        {
            try
            {
                ...
                else
                {
                    foreach (var x in request.ChiTietSanPhams)
                    {
                        _context.ChiTietSanPhams.Add(new ChiTietSanPham() { ID = x.IDChiTietSanPham, SoLuong = x.SoLuong.Value, GiaBan = x.GiaBan.Value, NgayTao = DateTime.Now, TrangThai = 2, IDSanPham = request.IDSanPham, IDPhongCach = x.IDMauSac.Value, IDDungTich = x.IDKichCo.Value, Ma = RemoveUnicode(request.Ma + x.TenMauSac.Replace(" ", "").ToUpper() + x.TenKichCo.ToUpper()) });
                    }
                }
                _context.SaveChanges();
                return true;
        ...
        public async Task<List<ChiTietSanPhamViewModelAdmin>> GetAllChiTietSanPhamAdmin(Guid idSanPham)
        {
            try
            {
                ...
                var lstChiTietSanPham = await (from a in _context.ChiTietSanPhams.Where(x => x.IDSanPham == idSanPham)
                                               join b in _context.PhongCachs on a.IDPhongCach equals b.ID
                                               join c in _context.DungTichs on a.IDDungTich equals c.ID
        ...
        public async Task<List<SanPhamBanHang>> GetAllSanPhamTaiQuay()
        {
            var result = await (from sp in _context.SanPhams.AsNoTracking().Where(c => c.TrangThai != 0)
                                ...
                                select new SanPhamBanHang()
                                {
                                    ...
                                    Ten = sp.Ten,
                                    MaSP = sp.Ma,
                                    Anh = (from ms in _context.PhongCachs.AsNoTracking()
                                           join a in _context.Anhs.Where(c => c.IDSanPham == sp.ID).AsNoTracking()
                                           on ms.ID equals a.IDPhongCach
                                           where ctsp != null && ctsp.IDPhongCach == ms.ID
        ...
        public async Task<ChiTietSanPhamBanHang> GetChiTietSPBHById(Guid idsp)
        {
            var lstMS = (from ctsp in _context.ChiTietSanPhams.AsNoTracking()
                         join ms in _context.PhongCachs.AsNoTracking() on ctsp.IDPhongCach equals ms.ID
                         where ctsp.IDSanPham == idsp && ctsp.TrangThai != 0
                         select new PhongCach
                         {
                             ID = ms.ID,
                             Ma = ms.Ma,
                             Ten = ms.Ten,
        ...
        public async Task<List<ChiTietCTSPBanHang>> GetChiTietCTSPBanHang(Guid idsp)
        {
            return await (from ctsp in _context.ChiTietSanPhams
                          ...
                          select new ChiTietCTSPBanHang()
                          {
                              ...
                              idMauSac = ctsp.IDPhongCach,
                              idKichCo = ctsp.IDDungTich,
                              SoLuong = ctsp.SoLuong,
                              Anh = (from ms in _context.PhongCachs
                                     join a in _context.Anhs on ms.ID equals a.IDPhongCach
                                     where ms.ID == ctsp.IDPhongCach && a.IDSanPham == ctsp.IDSanPham
