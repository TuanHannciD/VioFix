...
namespace AppView.Controllers
{
    public class NhomHuongController : Controller
    {
        ...
        public async Task<IActionResult> Show(int ProductPage = 1)
        {
            try
            {
                string apiUrl = $"https://localhost:7095/api/NhomHuong/GetAllNhomHuong";
                var response = await _httpClient.GetAsync(apiUrl);
                string apiData = await response.Content.ReadAsStringAsync();
                var users = JsonConvert.DeserializeObject<List<NhomHuong>>(apiData);
                return View(new PhanTrangNhomHuong
                {
                    listNv = users
        ...
        public async Task<IActionResult> SearchTheoTen(string? Ten, int ProductPage = 1)
        {
            try
            {
                ...
                string apiUrl = $"https://localhost:7095/api/NhomHuong/TimKiemNhomHuong?name={Ten}";
                var response = await _httpClient.GetAsync(apiUrl);
                string apiData = await response.Content.ReadAsStringAsync();
                var users = JsonConvert.DeserializeObject<List<NhomHuong>>(apiData);
                if (users.Count == 0)
                ...
                return View("Show", new PhanTrangNhomHuong
                {
                    listNv = users
                             .Skip((ProductPage - 1) * PageSize).Take(PageSize),
                    PagingInfo = new PagingInfo
        ...
        public async Task<IActionResult> Create(NhomHuong nh)
        {
            try
            {
                nh.TrangThai = 1;
                string apiUrl = $"https://localhost:7095/api/NhomHuong/ThemNhomHuong?ten={nh.Ten}";
        ...
        {
            string apiUrl = $"https://localhost:7095/api/NhomHuong/GetNhomHuongById?id={id}";
            var response = await _httpClient.GetAsync(apiUrl);
            string apiData = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> Edit(Guid id, NhomHuong nv)
        {
            ...
            {
                nv.TrangThai = 1;
                string apiUrl = $"https://localhost:7095/api/NhomHuong/{id}?ten={nv.Ten}";
        ...
        {
            string apiUrl = $"https://localhost:7095/api/NhomHuong/{id}";
