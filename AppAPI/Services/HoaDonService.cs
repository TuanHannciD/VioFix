                ...
                if (chiTietHoaDons != null)
                {
                    ...
                    if (reposHoaDon.Add(hoaDon1))
                    {
                        foreach (var x in chiTietHoaDons)
                        {
                            ...
                            //Tâm
                            donMua.GioHangs.Add(new GioHangRequest() { Anh = context.Anhs.First(y => y.IDSanPham == CTsanPham.IDSanPham && y.IDDungTich == CTsanPham.IDDungTich).DuongDan, SoLuong = x.SoLuong, DonGia = x.DonGia, Ten = context.SanPhams.First(y => y.ID == CTsanPham.IDSanPham).Ten, NhomHuong = context.NhomHuongs.First(y => y.ID == CTsanPham.IDNhomHuong).Ten, DungTich = context.DungTichs.First(y => y.ID == CTsanPham.IDDungTich).Ten });
                        }
                        //tích điểm, dùng điểm
        ...
        public ChiTietHoaDonQL GetCTHDByID(Guid idhd)
        {
            try
            {
                var lsthdct = (from cthd in context.ChiTietHoaDons
                               ...
                               join ms in context.DungTichs on ctsp.IDDungTich equals ms.ID
                               join kc in context.NhomHuongs on ctsp.IDNhomHuong equals kc.ID
