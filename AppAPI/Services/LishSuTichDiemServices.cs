        ...
        public async Task<List<DonMuaChiTietViewModel>> getAllDonMuaChiTiet(Guid idHoaDon)
        {
            try
            {
                var lstDonMuaCT = await(from c in context.ChiTietHoaDons.Where(p => p.IDHoaDon == idHoaDon)
                                      ...
                                      join f in context.NhomHuongs on e.IDNhomHuong equals f.ID
                                      join g in context.DungTichs on e.IDDungTich equals g.ID
                                      ...
                                      select new DonMuaChiTietViewModel()
                                      {
                                         ...
                                         TenMau = g.Ten,
                                         TenSanPham = i.Ten,
                                         DuongDan = context.Anhs.First(x => x.IDDungTich == g.ID && x.IDSanPham == i.ID).DuongDan,
        ...
        public async Task<ChiTietHoaDonDanhGiaViewModel> getCTHDDanhGia(Guid idcthd)
        {
            try
            {
                var getCTHDDanhGia = (from a in context.ChiTietHoaDons.Where(p=>p.ID == idcthd)
                                        ...
                                        join c in context.NhomHuongs on b.IDNhomHuong equals c.ID
                                        join d in context.DungTichs on b.IDDungTich equals d.ID
                                        ...
                                        select new ChiTietHoaDonDanhGiaViewModel()
                                        {
                                            ...
                                            TenMau = d.Ten,
                                            TenKichThuoc = c.Ten,
                                            DuongDan = context.Anhs.First(x => x.IDDungTich == b.IDDungTich && x.IDSanPham == e.ID).DuongDan,
                                            IDHoaDon = g.ID
